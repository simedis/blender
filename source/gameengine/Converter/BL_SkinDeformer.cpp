...
void BL_SkinDeformer::Relink(std::map<SCA_IObject *, SCA_IObject *>& map)
{
	...
	if (armobj) {
		if (m_armobj)
			m_armobj->UnregisterDeformer(this);
		m_armobj = static_cast<BL_ArmatureObject *>(armobj);
		m_armobj->RegisterDeformer(this);
	}
...
void BL_SkinDeformer::BlenderDeformVerts()
{
	...
	armature_deform_verts(par_arma, m_objMesh, nullptr, m_transverts, nullptr, m_totvert, m_deformflags, nullptr, nullptr);
...
bool BL_SkinDeformer::UpdateInternal(bool shape_applied)
{
	...
	if (PoseUpdated()) {
		...
		m_lastArmaUpdate = m_armobj->GetLastFrameAction();
