...
void KX_KetsjiEngine::RenderShadowBuffers(KX_Scene *scene, std::vector<KX_Camera *> cameras)
{
	...
	for (CListValue::iterator<KX_LightObject> it = lightlist->GetBegin(), end = lightlist->GetEnd(); it != end; ++it) {
		...
		for (std::vector<KX_Camera *>::iterator it = cameras.begin(), end = cameras.end(); it != end; ++it) {
			...
			raslight->GetShadowBox(lightfrustum, raslight->GetShadowMatrix());

			if (raslight->m_showShadowFrustum) {
				m_lightsFrustum.push_back(lightfrustum);
			}
...
void KX_KetsjiEngine::PostRenderScene(KX_Scene *scene, unsigned short target)
{
	...
	if (m_lightsFrustum.size() > 0) {
		for (std::vector<MT_Vector3 *>::iterator it = m_lightsFrustum.begin(); it != m_lightsFrustum.end();) {
			m_rasterizer->DrawDebugTransparentBoxes(*it);
			it = m_lightsFrustum.erase(it);
		}
	}
